...
- **La ligne dit où le métier atterrit.** `Après ces crafts : niveau 95 → 105`,
  ...
  L'info-bulle donne l'XP gagnée, l'XP d'arrivée et le reste du palier.
- **Les crafts se chaînent.** Chaque recette part du niveau où les précédentes
  ont amené le métier, dans l'ordre des cartes. Monter Bûcheron de 40 à 60 avec
  des Substrats de Bocage rend les Substrats de Futaie craftables, et ils se
  chiffrent alors au niveau 60. **L'XP réelle reste la base et n'est jamais
  réécrite** ; l'XP simulée vit à côté, le temps de l'affichage, et la carte du
  métier l'annonce sous la vraie : `après la session : niveau 60`.
  - L'ordre est celui des cartes parce qu'il est visible et réarrangeable. Un
    autre ordre donnerait d'autres niveaux, et il n'y en a pas de juste dans
    l'absolu.
  - Les sous-crafts n'y contribuent pas : leur quantité vient de l'analyse, qui a
    besoin des quantités que ce calcul produit. La simulation **sous-estime**
    donc le niveau atteint plutôt que l'inverse — le bon sens de l'erreur quand
    on décide d'acheter des ressources.
...
| 21 08 2026 | **Les crafts d'une session chaînent leur XP.** Chaque craft était projeté depuis l'XP réelle du métier, donc tous depuis le même point : une session qui monte Bûcheron de 40 à 60 avec des Substrats de Bocage continuait à marquer les Substrats de Futaie hors de portée et à les chiffrer au niveau 40. `chainerLXPDeLaSession` parcourt la session une fois, dans l'ordre des cartes, et rend pour chaque ligne l'XP d'où elle part et la quantité que son objectif réclame. **Une seule passe, et c'est le point délicat** : la quantité d'un craft dépend du niveau où il commence, et le niveau où commence le suivant dépend de cette quantité — les calculer séparément ferait afficher un compte de crafts qui ne mène pas au niveau annoncé juste à côté. L'XP réelle n'est jamais réécrite ; la carte du métier montre `après la session : niveau 60` sous la vraie. Les sous-crafts ne contribuent pas au chaînage, leur quantité venant de l'analyse qui a besoin de ce calcul : la simulation sous-estime donc le niveau atteint plutôt que l'inverse. Au passage, `installerLaTableDesMetiers` permet enfin de tester la logique de session sous Node, où `fetch` n'a aucun fichier à servir. |
